...
* Wählen Sie per Klick auf **Metadaten anzeigen** eine Datenquelle aus und klicken anschließend auf den Reiter **Anwendungen**,
...
A: Bitte stellen Sie sicher, dass Sie die php-gd-Bibliothek installiert haben. Wir empfehlen, die Erweiterung bereits vor dem Download von Mapbender zu installieren.
Auf Linux-Systemen können Sie sie wie folgt nachinstallieren:
...
A: Das Problem kann beim Zugriff auf einen Dienst über https auftreten. Wenn beim Laden oder Aktualisieren einer OGC WMS-Datenquelle auf Windows-basierten Mapbender-Servern ein SSL-Zertifikatsproblem auftritt, müssen Sie die *cacert.pem*-Datei aktualisieren und in Ihrer *php.ini* auf sie verweisen.
...
.. note:: Es gibt eine *cacert.pem*-Datei, die alle vertrauenswürdigen Zertifizierungsstellen auflistet. *cacert.pem* ist base64-kodiert und definiert alle vertrauenswürdigen Zertifizierungsstellen. Sie können die Datei `hier <https://curl.haxx.se/docs/caextract.html>`_ herunterladen.
...
Weitere Informationen finden Sie in der `PHP-Dokumentation <https://www.php.net/manual/en/curl.configuration.php>`_.
